import { DynamoDBClient, CreateTableCommand, DeleteTableCommand,PutItemCommandInput } from "@aws-sdk/client-dynamodb";
import { DeleteCommand, DynamoDBDocumentClient, GetCommand, GetCommandInput, PutCommand, QueryCommandInput } from "@aws-sdk/lib-dynamodb";
  ...
  describe("DynamoDbResultClient", () => {
    ...
    describe('.putItem', () => {
        ...
        test("DynamoDBへのアイテムの追加が成功した場合、追加したアイテムが返される", async () => {
            const item = marshall({ [partitionKey]: "1", [sortKey]: "1", name: "John Doe" })
            ...
            expect(retrievedItem).toEqual({ [partitionKey]: "1", [sortKey]: "1", name: "John Doe" });
        });
    ...
    describe('.deleteItem', () => {
      ...
      describe('.queryItem', () => {
        ...
        test('指定したパーティションキーに対応するアイテムが返される', async () => {
            ...
            const items = result._unsafeUnwrap().Items?.map(item => unmarshall(item))
            expect(items).toHaveLength(2);
            expect(items).toContainEqual({ [partitionKey]: '1', [sortKey]: '1', name: 'John' });
            expect(items).toContainEqual({ [partitionKey]: '1', [sortKey]: '2', name: 'Jane' });
        });
